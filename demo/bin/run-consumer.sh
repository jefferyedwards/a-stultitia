#!/usr/bin/env bash
#
# Launches the TimeOfDay Consumer.
...
CLASSPATH="${PROJECT_DIR}/consumer/target/consumer-1.0.0-SNAPSHOT.jar"
CLASSPATH="${CLASSPATH}:${IDL_JAR}"
CLASSPATH="${CLASSPATH}:${RTI_JAR_DIR}/nddsjava.jar"
CLASSPATH="${CLASSPATH}:${PROJECT_DIR}/consumer/target/lib/*"
...
cd "$DEMO_DIR"
java -cp "$CLASSPATH" net.edwardsonthe.consumer.TimeOfDayConsumer "$@"
